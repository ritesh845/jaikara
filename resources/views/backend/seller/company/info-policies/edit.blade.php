		...
		<form method="post" action="{{route('info_policy_update')}}">
			...
			<div class="row">
				...
				<div class="col-md-12 form-group">
					...
					<input class="mr-2 ml-2" type="checkbox" name="payment_curre[]" value="{{$currencie->curr_id}}" {{ $inforPolicies ? (in_array($currencie->curr_id,json_decode($inforPolicies->payment_curre) ) ? 'checked' : '') :'' }} > <label for="1">{{$currencie->curr_name}}</label>
					@endforeach
				</div>
				<div class="col-md-12 form-group">
					...
					<input class="mr-2 ml-2" type="checkbox" name="delivery_terms[]" value="{{$deliveryTerm->dlvry_trms_id}}" {{ $inforPolicies ? (in_array($deliveryTerm->dlvry_trms_id,json_decode($inforPolicies->delivery_terms) ) ? 'checked' : '') :'' }} > 
				...
				<div class="col-md-12 form-group">
					...
					<input class="mr-2 ml-2" type="checkbox" name="payment_method[]" value="{{$paymentMethod->pay_md_id}}" {{ $inforPolicies ? (in_array($paymentMethod->pay_md_id,json_decode($inforPolicies->payment_method) ) ? 'checked' : '') :'' }}> 
					<label for="22">{{$paymentMethod->shrt_name}}</label>
				@endforeach
				</div>
				<div class="col-md-12 form-group">
					<label>Spoken Language <a  title="Accepted Payment Currency" class="fa fa-question-circle"></a></label><br>
					@foreach($languages as $language)
					<input  class="mr-2 ml-2"  type="checkbox" name="spoken_lang[]" value="{{$language->lang_id}}" {{ $inforPolicies ? (in_array($language->lang_id,json_decode($inforPolicies->spoken_lang) ) ? 'checked' : '' ): '' }}> 
				...
				<div class="col-md-12 form-group">
					...
					<table width="100%" cellspacing="2" cellpadding="2" border="0">
						...
						@foreach($certifications as $certification)
							<tr>
								<td>
									<input class="mr-2 ml-2" type="checkbox" name="certification[]" value="{{$certification->cert_id}}" {{ $inforPolicies ? (in_array($certification->cert_id,json_decode($inforPolicies->certification) ) ? 'checked' : '') :'' }}>
								</td>
				...
				<div class="col-md-12 form-group">
					...
					<input class="mr-2 ml-2" type="radio" name="escrow_service" value="1" {{ $inforPolicies ? ($inforPolicies->escrow_service == 1 ? 'checked' :'' ) : '' }}>
					<label for="radio">Yes </label>
					  /    
					<input type="radio" name="escrow_service" value="0" {{$inforPolicies ? ($inforPolicies->escrow_service == 0 ? 'checked' :'' ) : '' }}>
